...
!!
!! Rationale: the previous version of this file read fixtures from
!! data/tables/cpp_legacy/, a directory that no longer ships with the project
!! (the tables were converted to the native binary format in
!! data/tables/fortran_native/, and data/ is not versioned). With the fixtures
!! gone, read_cpp_table correctly reported ERROR_FILE_NOT_FOUND and left the
!! table components unallocated, but the tests ignored the status and went on to
!! dereference the unallocated components (e.g. table_in%n_grid(1)), which
